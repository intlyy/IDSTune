...
def get_features_list(model,messages1, message2):
    messages=[  
    ...
    with open("features_selected.json", "w", encoding="utf-8") as f:
        f.write(choice.message.content)

if __name__ == "__main__":
    model = "gpt-4.1"
    downstream_task = "index recommendation"
    with open("features_all.json", "r", encoding="utf-8") as f:
        features_all = f.read()
    message2 = messages2.format(downstream_task=downstream_task, features_all=features_all)
    get_features_list(model,messages1, message2)